...
class TaskLogic:
    ...
    def compute_target_positions(self, env_ids: torch.Tensor) -> torch.Tensor:
        target_positions = torch.zeros((len(env_ids), 3), device=self.device)
        for i, env_idx in enumerate(env_ids):
...
class FrankaReachEnv(ManagerBasedRLEnv):
    ...
    def _setup_scene(self):
        super()._setup_scene()
        self.contact_sensor = ContactSensor(prim_path="{ENV_REGEX_NS}/Robot/panda_link(5|6|7|hand)", force_threshold=1.0)
